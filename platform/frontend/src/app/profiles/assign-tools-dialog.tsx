...
interface AssignToolsDialogProps {
  profile: archestraApiTypes.GetAllAgentsResponses["200"][number];
  open: boolean;
...
export function AssignToolsDialog({
  ...
  // Fetch currently assigned tools for this profile (use getAllAgentTools to get credentialSourceMcpServerId)
  const { data: allAgentTools } = useAllAgentTools({});
  const profileToolRelations = useMemo(
    () => allAgentTools?.data?.filter((at) => at.agent.id === profile.id) || [],
    [allAgentTools, profile.id],
  );

  // Track selected tools with their credentials, execution source, and profile-tool IDs
  ...
  // Initialize selected tools when profile tools load
  useEffect(() => {
    if (profileToolRelations) {
      setSelectedTools(
        profileToolRelations.map((at) => ({
          toolId: at.tool.id,
  ...
  const handleSave = useCallback(async () => {
    ...
    const currentToolIds = new Set(profileToolRelations.map((at) => at.tool.id));
    ...
    const toUnassign = profileToolRelations.filter(
      (at) => !selectedToolIds.has(at.tool.id),
    );
    const toUpdate = selectedTools.filter((tool) => {
      if (!tool.profileToolId) return false;
      const current = profileToolRelations.find(
    ...
    try {
      ...
      for (const tool of toUpdate) {
        if (tool.profileToolId) {
          await patchAgentTool.mutateAsync({
            id: tool.profileToolId,
            credentialSourceMcpServerId: tool.credentialsSourceId || null,
            executionSourceMcpServerId: tool.executionSourceId || null,
          });
        }
      ...
      toast.success(`Successfully updated tools for ${profile.name}`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Assign Tools to {profile.name}</DialogTitle>
          <DialogDescription>
