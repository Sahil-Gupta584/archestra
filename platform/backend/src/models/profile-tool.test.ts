...
describe("ProfileToolModel.findAllPaginated", () => {
  describe("Pagination", () => {
    test("returns paginated results with correct metadata", async ({
      ...
      // Test first page
      const page1 = await ProfileToolModel.findAllPaginated(
        { limit: 2, offset: 0 },
        undefined,
      ...
      // Test second page
      const page2 = await ProfileToolModel.findAllPaginated(
        { limit: 2, offset: 2 },
        undefined,
        { excludeArchestraTools: true },
      );
      ...
      // Test last page
      const page3 = await ProfileToolModel.findAllPaginated(
        { limit: 2, offset: 4 },
        undefined,
        { excludeArchestraTools: true },
      );
    ...
    test("respects custom page size", async ({
      ...
      const result = await ProfileToolModel.findAllPaginated(
        { limit: 3, offset: 0 },
        undefined,
        { excludeArchestraTools: true },
      );
  ...
  describe("Sorting", () => {
    ...
    test("sorts by origin (MCP vs LLM Proxy)", async ({
      ...
      const resultAsc = await ProfileToolModel.findAllPaginated(
        { limit: 10, offset: 0 },
    ...
    test("sorts by allowUsageWhenUntrustedDataIsPresent", async ({
      ...
      const resultDesc = await ProfileToolModel.findAllPaginated(
        { limit: 10, offset: 0 },
        {
          sortBy: "allowUsageWhenUntrustedDataIsPresent",
          sortDirection: "desc",
  ...
  describe("Filtering", () => {
    ...
    test("search is case-insensitive", async ({
      ...
      const resultLower = await ProfileToolModel.findAllPaginated(
        { limit: 10, offset: 0 },
        undefined,
        { search: "readfile" },
      );

      const resultUpper = await ProfileToolModel.findAllPaginated(
        { limit: 10, offset: 0 },
        undefined,
        { search: "READFILE" },
      );
