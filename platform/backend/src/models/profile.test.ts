...
describe("ProfileModel", () => {
  ...
  describe("Access Control", () => {
    test("can create agent with team assignments", async ({
      ...
      const agent = await ProfileModel.create({
        name: "Test Agent",
        teams: [team.id],
      });
    ...
    test("admin can see all agents", async ({ makeAdmin }) => {
      ...
      await ProfileModel.create({ name: "Agent 1", teams: [] });
      await ProfileModel.create({ name: "Agent 2", teams: [] });
      await ProfileModel.create({ name: "Agent 3", teams: [] });

      const agents = await ProfileModel.findAll(admin.id, true);
    ...
    test("member only sees agents in their teams", async ({
      ...
      const agents = await ProfileModel.findAll(user1.id, false);
    ...
    test("member with no team membership sees empty list", async ({
      ...
      const agents = await ProfileModel.findAll(user2.id, false);
    ...
    test("findById returns agent for admin", async ({ makeAdmin }) => {
      ...
      const foundAgent = await ProfileModel.findById(agent.id, admin.id, true);
    ...
    test("findById returns agent for user in assigned team", async ({
      ...
      const foundAgent = await ProfileModel.findById(agent.id, user.id, false);
      expect(foundAgent).not.toBeNull();
      expect(foundAgent?.id).toBe(agent.id);
    });
    ...
    test("findById returns null for user not in assigned teams", async ({
      ...
      const foundAgent = await ProfileModel.findById(agent.id, user2.id, false);
    ...
    test("update syncs team assignments correctly", async ({
      ...
      const updatedAgent = await ProfileModel.update(agent.id, {
        teams: [team2.id],
      });
    ...
    test("teams is always populated in responses", async ({
      ...
      const foundAgent = await ProfileModel.findById(agent.id);
  ...
  describe("Label Ordering", () => {
    ...
    test("findAll returns labels in alphabetical order for all agents", async () => {
      ...
      const agents = await ProfileModel.findAll();
  ...
  describe("Pagination", () => {
    ...
    test("pagination works correctly when agents have many tools", async ({
      ...
      const agent3 = await ProfileModel.create({
        name: "Agent 3",
        teams: [],
      });
      await ProfileModel.create({
